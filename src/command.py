...
def input_command(state):
    ...
    while active:
        ...
        if command_key == "d" and can_move(1, 0, state.g, state):  # move right
            move_right(state)

        elif command_key == "jd" and can_jump(2, 0, state.g, state):
            jump_right(state)

        elif command_key == "w" and can_move(0, -1, state.g, state): # move up
            move_up(state)

        elif command_key == "jw" and can_jump(0, -2, state.g, state):
            jump_up(state)

        elif command_key == "a" and can_move(-1, 0, state.g, state): # move left
            move_left(state)

        elif command_key == "ja" and can_jump(-2, 0, state.g, state):
            jump_left(state)

        elif command_key == "s" and can_move(0, 1, state.g, state): # move down
            move_down(state)

        elif command_key == "js" and can_jump(0, 2, state.g, state):
            jump_down(state)
